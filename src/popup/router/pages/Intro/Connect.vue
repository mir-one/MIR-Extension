  ...
  <div class="overlay container">
    <div class="overlay_inner">
      <div class="overlay-message">
        ...
        <h4 class="text-center">Searching for your MIR.Cube Node</h4>
      </div>
...
export default {
  ...
  data() {
    return {
      baseUrl: 'http://mir-cube-node.local',
      pending: true,
      connected: false,
  ...
  async created() {
    try {
      ...
      if (!this.clickedManualConfig) {
        this.$notify({ group: 'alerts', type: 'success', title: 'Connected', text: 'Connected to mir-cube-node.local.' });
      }
    } catch (err) {
      // Don't show error if using manual override
      if (!this.clickedManualConfig) {
        this.$notify({ group: 'alerts', type: 'error', title: 'Connection Failed', text: 'No response from mir-cube-node.local. Try entering the IP manually.' });
      }
    } finally {
      if (!this.clickedManualConfig) {
        this.pending = false;
